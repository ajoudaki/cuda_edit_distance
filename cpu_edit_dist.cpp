...
dist_t edit_distance_lowmem(char *s1, char *s2, unsigned int l1, unsigned int l2) {
    ...
    for(int i=1;i<=l2;i++) { // d1 = row dist[i,:], d0 = row  [i-1,:]
        d0[0] = i-1;
        d1[0] = i;
        for (int j=1;j<=l1;j++) {
            int track, t;
...
int main() {
    ...
    h1[len] = h2[len] = 0;
    compare_times(h1, h2, len);
    test_equality(h1, h2, len);
}